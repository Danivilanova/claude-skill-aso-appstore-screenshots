...
- **Orchestration is part of the skill, not the environment**: the delegation model lives in SKILL.md and uses only `subagent_type: "general-purpose"` (or `"Explore"` when present) with an explicit `model`, so a clone works in vanilla Claude Code. Two rules there exist for correctness rather than speed: back-translation runs in a fresh agent that never sees the English source, and no subagent may call generate_ai.py with parameters it wrote itself or "fix" a headline that `compose.py --strict` rejected.
